...
def generate_applicant_dataset(num_rows=ROWS):
    ...
    # employment_status: Full-time(70%), Part-time(20%), Unemployed(10%) - Adjusted for better approval rates
    emp_choices = ["Full-time", "Part-time", "Unemployed"]
    emp_probs   = [0.7, 0.2, 0.1]  # Increased Full-time probability
...
def generate_credit_dataset(num_rows=ROWS):
    ...
    # credit_score from Normal(700,40), clipped to [300,900] - Adjusted for better approval rates
    ...
    # Adjusted payment history probabilities for better approval rates
    pmt_choices = ["On Time", "Late<30", "Late 30-60", "Late>60"]
    pmt_probs   = [0.75,     0.15,        0.07,        0.03]  # Increased On Time probability
...
def finalize_approval(df):
    ...
    def approve_row(row):
        ...
        # Otherwise random with higher probability of approval
        return np.random.choice([0,1], p=[0.3, 0.7])  # Increased approval probability
