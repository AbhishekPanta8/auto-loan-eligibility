...
def generate_applicant_dataset(num_rows=ROWS):
    ...
    requested_amount = np.random.uniform(1000, 50000, num_rows)
...
def finalize_approval(df):
    ...
    dti = (monthly_debt + (df["requested_amount"] * 0.03)) / monthly_income * 100.0
    df["DTI"] = dti
...
def finalize_approved_amount_and_interest(df):
    ...
    Then compute interest_rate with:
      base random in [Prime + 0.5 .. Prime + 6.75] (using 3.0 as base), 
      + or - adjustments:
        - credit_score>=750 => -1%
        - credit_score 500..659 => +2%, <500 => +4%
        - if DTI>30 => +1%
        - if payment_history='Late>60' => +2%
        - if num_open_accounts > 5 => +1%
      clamp [3,15]
    """
    ...
    def compute_interest_rate(row):
        # base Prime + 0.5 to Prime + 6.75 (using 3.0 as base rate)
        base_rate = np.random.uniform(3.0, 9.25)  # 3.0 + 0.5 to 3.0 + 6.75

        score = row["credit_score"]
        dti   = row["DTI"]
        pmt   = row["payment_history"].lower()
        open_accts = row["num_open_accounts"]
...
def introduce_noise(df, noise_frac=0.05, columns=None):
    """
    Introduces noise in 5-10% of the data for realism
    """
    if columns is None:
        columns = []
    ...
    for col in columns:
        ...
        if col == "annual_income":
            ...
            for nv in noise_values:
                if nv == "low":
                    new_vals.append(np.random.uniform(10000, 20000))
                else:
                    new_vals.append(np.random.uniform(200000, 210000))
...
def main():
    ...
    # 8) Save
    df_applicant.to_csv("applicant_dataset_org.csv", index=False)
    df_credit.to_csv("third_party_dataset_org.csv", index=False)
    df_merged.to_csv("synthetic_loan_applications_org.csv", index=False)
